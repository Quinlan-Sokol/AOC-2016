...
public class Day4 {
    ...
    {
        String[] rooms = {"gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]",
...
"jyfvnlupj-ibuuf-svnpzapjz-851[gmsnf]"};
    ...
    public static void part2(String[] rooms)
    {
        for(String str : rooms)
        {
            String[] parts = str.split("-");
            String number = parts[parts.length - 1].split("\\[")[0];
            String name = "";
            
            for(int i = 0; i < parts.length - 1; i++)
            {
                for(int j = 0; j < parts[i].toCharArray().length; j++){
                    char c = parts[i].toCharArray()[j];
    ...
    public static void part1(String[] rooms)
    {
        int sum = 0;
        for(String str : rooms)
        {
            String[] parts = str.split("-");
            String checksum = parts[parts.length - 1].split("\\[")[1].replace("]", "");
            String number = parts[parts.length - 1].split("\\[")[0];
            ArrayList<String> name = new ArrayList();
            for(int i = 0; i < parts.length - 1; i++)
            {
                for(int j = 0; j < parts[i].toCharArray().length; j++){
                    name.add(Character.toString(parts[i].toCharArray()[j]));
                }
            }
            Collections.sort(name);
            
            //FINDING CHECKSUM LETTERS
            String nameChecksum = "";
            for(int k = 0; k < 5; k++)
            {
                String remove = "";
                ...
                for(String s : name)
                {
                    if(Collections.frequency(name, s) > max){
                        max = Collections.frequency(name, s);
                        remove = s;
                    }
                }
                nameChecksum += remove;
                while(name.contains(remove)){
                    name.remove(remove);
                }
            }
            
            if(checksum.equals(nameChecksum)){
