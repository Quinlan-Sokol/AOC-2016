...
public class Day10 {
    ...
    {
        String[] instr = {"bot 76 gives low to bot 191 and high to bot 21",
...
"bot 71 gives low to bot 152 and high to bot 121"};
        ...
        for (String str : instr) {
            if(str.contains("value")){
                bots.get(Integer.parseInt(str.split(" ")[5])).chips.add(Integer.parseInt(str.split(" ")[1]));
            }
        }
        //OUTER:
        int count = 0;
        while(count<10000){count++;
            for(Bot b : bots){
                if(b.hasTwo()){
                    for (String str : instr) {
                        if(str.contains("gives") && Integer.parseInt(str.split(" ")[1]) == b.index){
                             /*if(b.chips.contains(61) && b.chips.contains(17)){
                             ...
                             int index1 = Integer.parseInt(str.split(" ")[6]);
                             int index2 = Integer.parseInt(str.split(" ")[11]);
                             //LOW
                             if(str.split(" ")[5].equals("bot")){
                                 bots.get(index1).chips.add(low);
                             }
                             else if(str.split(" ")[5].equals("output")){
                                 outputs.get(index1).chips.add(low);
                             }
                             //HIGH
                             if(str.split(" ")[10].equals("bot")){
                                 bots.get(index2).chips.add(high);
                             }
                             else if(str.split(" ")[10].equals("output")){
                                 outputs.get(index2).chips.add(high);
                             }
                        }
                    }
        ...
        System.out.println(outputs.get(0).chips.get(0) * outputs.get(1).chips.get(0) * outputs.get(2).chips.get(0));
    }
}
class Bot
{
